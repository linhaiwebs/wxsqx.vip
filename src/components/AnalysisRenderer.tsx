...
const parseNumber = (text: string): ReactNode[] => {
  ...
  while ((match = numberRegex.exec(text)) !== null) {
    ...
    parts.push(
      <span key={match.index} className="text-qsdj-magenta-dark font-bold text-lg font-display">
        {match[0]}
      </span>
    );
  ...
  return (
    <div className="leading-relaxed space-y-2 text-sm">
      {parsedLines.map((line, index) => {
        if (line.type === 'bold') {
          return (
            <div key={index} className="font-bold text-qsdj-on-bg mt-4 mb-2 font-display">
              {line.content}
            </div>
          );
        }
        return (
          <div key={index} className="text-qsdj-on-surface-variant font-body">
            {line.content}
          </div>
        );
      })}
