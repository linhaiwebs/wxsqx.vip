...
const parseNumber = (text: string): ReactNode[] => {
  ...
  while ((match = numberRegex.exec(text)) !== null) {
    ...
    parts.push(
      <span key={match.index} className="text-[#ea580c] font-bold text-lg font-headline">
        {match[0]}
      </span>
    );
  ...
  return (
    <div className="leading-relaxed space-y-2 text-sm">
      {parsedLines.map((line, index) => {
        if (line.type === 'bold') {
          return (
            <div key={index} className="font-bold text-black mt-4 mb-2 font-headline">
              {line.content}
            </div>
          );
        }
        return (
          <div key={index} className="text-on-surface font-body">
            {line.content}
          </div>
        );
      })}
