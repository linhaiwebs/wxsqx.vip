...
const parseNumber = (text: string): ReactNode[] => {
  ...
  while ((match = numberRegex.exec(text)) !== null) {
    ...
    parts.push(
      <span key={match.index} className="text-xnw-cyan font-bold text-lg font-display">
        {match[0]}
      </span>
    );
  ...
  return (
    <div className="leading-relaxed space-y-2 text-sm">
      {parsedLines.map((line, index) => {
        if (line.type === 'bold') {
          return (
            <div key={index} className="font-bold text-xnw-on-bg mt-4 mb-2 font-display">
              {line.content}
            </div>
          );
        }
        return (
          <div key={index} className="text-xnw-on-surface-variant font-body">
            {line.content}
          </div>
        );
      })}
