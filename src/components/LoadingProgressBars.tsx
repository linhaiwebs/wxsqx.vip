...
export default function LoadingProgressBars({ isVisible }: LoadingProgressBarsProps) {
  ...
  const stages = [
    { label: 'DATA_COLLECT', progress: s1, color: '#d2f000' },
    { label: 'AI_ANALYZE', progress: s2, color: '#ffffff' },
    { label: 'REPORT_GEN', progress: s3, color: '#d2f000' },
  ];
      ...
      {stages.map((stage, i) => (
        <div key={i}>
          <div className="flex justify-between items-end mb-1">
            <span className="font-display text-xs text-kw-on-surface-variant uppercase" style={{ letterSpacing: '0.1em', fontWeight: 500 }}>{stage.label}</span>
            <span className="font-display text-base text-kw-on-bg" style={{ fontWeight: 700 }}>{Math.floor(stage.progress)}%</span>
          </div>
          <div className="w-full h-1 bg-kw-surface-highest overflow-hidden">
            <div className="h-full transition-all duration-300" style={{ width: `${stage.progress}%`, backgroundColor: stage.color, boxShadow: `0 0 4px ${stage.color}` }} />
          </div>
        </div>
      ))}
    </div>
